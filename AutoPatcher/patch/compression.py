...
def compress(output_mod_asset_dir: Path, texconv_path: Path):
    header("Compressing textures...")
    for texture in Path(output_mod_asset_dir).glob("**/*.png"):

        current_item = Path("Assets") / texture.relative_to(output_mod_asset_dir)
